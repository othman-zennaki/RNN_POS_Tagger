...
int main(int argc, char **argv)
{
    ...
    //Modif 11/10 
    char tagSet_file[MAX_STRING];
    char trainTag_file[MAX_STRING];
    char validTag_file[MAX_STRING];

    char test_file[MAX_STRING];
    //Modif 15/09
    char testTag_file[MAX_STRING];
...
*/        // à faire ajouter les fichier dans le help
    ...
    //Modif 11/09 search for train file tag
    i=argPos((char *)"-train-tag", argc, argv);
    if (i>0) {
        if (i+1==argc) {
             printf("ERROR: trainTag file not specified!\n");
             return 0;
        }

        strcpy(trainTag_file, argv[i+1]);

        if (debug_mode>0)
        printf("trainTag file: %s\n", trainTag_file);

        f=fopen(trainTag_file, "rb");
        if(f==NULL) {
            printf("ERROR: trainTag file not found!\n");
	    return 0;
        }
         
        trainTag_set=1;
    }
    if (train_mode && !trainTag_set) {
	if (one_iter==0) {
	    printf("ERROR: trainTag file must be specified for training!\n");
    	    return 0;
    	}
      }
    ...
    if (i>0) {
        if (i+1==argc) {
             printf("ERROR: validTag file not specified!\n");
             return 0;
        }

        strcpy(validTag_file, argv[i+1]);

        if (debug_mode>0)
        printf("validTag file: %s\n", validTag_file);

        f=fopen(validTag_file, "rb");
        if(f==NULL) {
            printf("ERROR: validTag file not found!\n");
	    return 0;
        }
         
        validTag_set=1;
    }
    if (train_mode && !validTag_set) {
	if (one_iter==0) {
	    printf("ERROR: validTag file must be specified for training!\n");
    	    return 0;
    	}
      }
    
    //Modif 11/09 search for tagSet
    i=argPos((char *)"-tag-set", argc, argv);
    if (i>0) {
        if (i+1==argc) {
            printf("ERROR: tagSet file not specified!\n");
            return 0;
        }
        strcpy(tagSet_file, argv[i+1]);
        
        if (debug_mode>0)
        printf("tagset file: %s\n", tagSet_file);

        f=fopen(tagSet_file, "rb");
        if (f==NULL) {
            printf("ERROR: tagSet data file not found!\n");
            return 0;
        }
   
        tagSet_set=1;
     }
     if (train_mode && !tagSet_set) {
	if (one_iter==0) {
	    printf("ERROR: tagSet file must be specified for training!\n");
    	    return 0;
    	}
      }
    ...
    //Modif 15/09 search for test tag file
    i=argPos((char *)"-test-tag", argc, argv);
    if (i>0) {
         if (i+1==argc) {
            printf("ERROR: test tag file not specified!\n");
            return 0;
        }
        
        strcpy(testTag_file, argv[i+1]);
        if (debug_mode>0)
        printf("test tag file: %s\n", testTag_file);
        f=fopen(testTag_file, "rb");
        if (f==NULL) {
             printf("ERROR: test Tag file not found!\n");
             return 0;
        }
        testTag_set=1;

    }
    if (test_data_set && !testTag_set) {
	if (one_iter==0) {
	    printf("ERROR: testTag file must be specified for training!\n");
    	    return 0;
    	}
      }
    ...
    if (train_mode) {
    	...
    	//Modif 11/09
        model1.setTagSet(tagSet_file);
        model1.setTrainTag(trainTag_file);
        model1.setValidTag(validTag_file); 
    ...
    if (test_data_set && rnnlm_file_set) {
        ...
        //Modif 15/09
        model1.setTestTag(testTag_file);
        model1.setRnnLMFile(rnnlm_file);
        model1.setRandSeed(rand_seed);
