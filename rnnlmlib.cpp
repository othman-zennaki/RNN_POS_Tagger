...
//Modif 15/09
void CRnnLM::setTestTag(char *str)
{
    strcpy(testTag_file, str);
}

//Modif 11/09
void CRnnLM::setTagSet(char *str)
{
    strcpy(tagSet_file,str);
}
void CRnnLM::setTrainTag(char *str)
{
    strcpy(trainTag_file,str);
}
void CRnnLM::setValidTag(char *str)
{
    strcpy(validTag_file,str);
}
...
int CRnnLM::readWordIndex(FILE *fin)
{
    ...
    int tmp,a;                  
    readWord(word, fin);    
    if (feof(fin)) return-1; ///{printf("readWordIndex feof(fin) \n"); return -1;}
    ///printf("searchVocab avant %s \n",word); 
    ///if(!strcmp(word, "(")){   
    ///for(a=0; a<vocab_size; a++) 
    ///printf("Vocab %s \n",vocab[a].word );}
    //exit(0);
    tmp=searchVocab(word);
    ///printf("searchVocab succes \n");
    ///printf("readWordIndex /%s/ /%d/\n",word,tmp);     
    return tmp;
}

//Modif 11/09
int CRnnLM::searchTagSet(char *tag)
{
     int a;
   
     for (a=0; a<tagSet_size; a++){
         if (!strcmp(tag, tagSet[a].tag)){            
             return a;
         }
     }  
...
int CRnnLM::readTagIndex(FILE *fin,int wordIndexndex)
{
    char tag[MAX_STRING], word[MAX_STRING];    
    int tmp,a;
    ///printf("\n\n!!! word tag !!! ");
    tmp=readWordIndex(fin);
    ///printf("!!! tag !!! \n");
    readWord(tag, fin); 
    ///printf("wordIndexndex %d tmp %d \n",wordIndexndex, tmp);
    //printf("wi /%s/ /%d/ w /%s/ /%d/ t %s\n\n",vocab[wordIndexndex].word, wordIndexndex, vocab[tmp].word, tmp,tag);
    if (wordIndexndex!=tmp){
        //for(a=0; a<vocab_size; a++) 
        //printf("Vocab %s \n",vocab[a].word );        
        printf("train file and trainTag file are shifting in : %s %s \n",vocab[wordIndexndex].word, vocab[tmp].word);
        exit(1); 
    }
    
    //else {
                      
        while (searchVocab(word)!=0){         
           readWord(word, fin);
           if (feof(fin)) return-1; ///{printf(" readTagIndex feof(fin) \n"); return -1;}              
        }
    //}

    if (feof(fin)) return-1; ///{printf(" readTagIndex feof(fin) \n"); return -1;}     
    return searchTagSet(tag);
}
...
int CRnnLM::addWordToVocab(char *word)
{
    ...
    ///printf("addWordToVocab %s lay0 %d lay1 %d layc %d lay2 %d  \n", word,layer0_size,layer1_size,layerc_size,layer2_size);
    //hash=getWordHash(word);
...
void CRnnLM::sortVocab()
{
    int a, b, max;
    vocab_word swap;
    ///printf("sortVocab  lay0 %d lay1 %d layc %d lay2 %d  \n", layer0_size,layer1_size,layerc_size,layer2_size);
...
void CRnnLM::learnVocabFromTrainFile()    //assumes that vocabulary is empty
{
    ...
    FILE *fin;
    int a, i, train_wcn;
    ///printf("lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2);
    ...
    while (1) {
        readWord(word, fin);
        //printf("%s \n",word);
        ///printf("learnVocabFromTrainFile lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2);  
        if (feof(fin)) break;       
        train_wcn++;
        ///printf("learnVocabFromTrainFile1 lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2);
        i=searchVocab(word);
        ///printf("learnVocabFromTrainFile2 lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2);
        if (i==-1) {
...
//Modif 10/09 construire la representation phrastique 
void CRnnLM::buildPhRepresentation()
{
    char word[MAX_STRING];
    FILE *fin;
    int a, i, b;
    ...
    i=searchVocab((char *)"</s>");
    phRep_size=vocab[i].cn;
    //à faire if(phRep_size!=layer1_size)printf();
    //printf("phRep_size %d vocab_size %d layer1_size %d",phRep_size,vocab_size,layer1_size); 
    phRep=(int **)calloc(vocab_size, sizeof(int *));
    for (a=0; a<vocab_size; a++) {
    	phRep[a]=(int *)calloc(phRep_size, sizeof(int));
        for (b=0; b<phRep_size; b++){
	    phRep[a][b]=0;
	}
    }
    a=0;
    ...
    for (a=0; a<3; a++) {    	
        for (b=0; b<phRep_size; b++){
	    printf("%s  %d \n ",vocab[a].word,phRep[a][b]);
	}
    }exit(1);	*/
    fclose(fin); 	
}
//Modif 11/09
void CRnnLM::loadTagSet()
{
    char word[MAX_STRING];
    int a;
    FILE *fin;

    fin=fopen(tagSet_file,"rb");
    ...
    while (1) {
       ...
       if (strcmp(word,"</s>")){                          
             tagSet_size++;
             tagSet=(struct tagSet_tag *)realloc(tagSet, tagSet_size * sizeof(struct tagSet_tag));             
             strcpy(tagSet[tagSet_size-1].tag,word);
       }   
    }
    ...
    if (debug_mode>0) {
       ...
       printf("PhRepresentation size: %d\n",phRep_size);
...
void CRnnLM::initNet()
{
    ...
    layer0_size=phRep_size+layer1_size;
    layer2_size=tagSet_size;
    //printf("layer0_size %d phRep_size %d layer1_size %d tagSet_size %d vocab_size %d \n",layer0_size,phRep_size,layer1_size,tagSet_size,vocab_size);
    //exit(0);
...
printf("InitNet lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p vocab%p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2,vocab);
}
...
void CRnnLM::saveNet()       //will save the whole network structure                                                        
{
    ...
    //Modif 10/09 enregistrer les tailles des couches du reseau
    ...
    fprintf(fo, "\nTagSet:\n");
    for (a=0; a<tagSet_size;a++) fprintf(fo, "\t%s\n",tagSet[a].tag);   
...
void CRnnLM::restoreNet()    //will read whole network structure
{
    ...
    //
    tagSet_size=layer2_size;
    //Modif 14/09 load tagset
    if (tagSet!=NULL) free(tagSet);
    tagSet=(struct tagSet_tag *)calloc(tagSet_size, sizeof(struct tagSet_tag));
    goToDelimiter(':', fi);
    for(a=0;a<tagSet_size;) {
          readWord(tagSet[a].tag,fi);
          if(strcmp(tagSet[a].tag,"</s>")){          	
                //printf("%s\n",tagSet[a].tag);
                a++;
          }
    }     
...
void CRnnLM::computeNet(int last_word, int word)
{
    ...
    /*if(last_word==-1 || word==-1){
          printf("ComputeNet word or tag = -1 word: %d tag: %d\n",last_word,word);exit(1);}*/
    //printf("Computnet in %d \n",phRep_size);
    for (a=0; a<phRep_size; a++) neu0[a].ac=0;
    
   //Modif 12/09 load phrastique representation
    if (last_word!=-1){
        for(a=0; a<phRep_size; a++)
            neu0[a].ac=phRep[last_word][a];             
    }
    else{
        for(a=0; a<phRep_size; a++)
            neu0[a].ac=0;
    ...
    //Modif 12/09
    matrixXvector(neu1, neu0, syn0, layer0_size, 0, layer1_size, 0, layer0_size-layer1_size, 0);
    ...
    */
    ...
    if (word!=-1) {
        for (c=0; c<layer2_size; c++) neu2[c].ac=0;
        if (layerc_size>0) {
	    matrixXvector(neu2, neuc, sync, layerc_size, 0, layer2_size, 0, layerc_size, 0);
	}
	else
	{
            matrixXvector(neu2, neu1, syn1, layer1_size, 0, layer2_size, 0, layer1_size, 0);
    ...
    // à faire trouver une solution pour la connexion directe
    /*
...
void CRnnLM::learnNet(int last_word, int word)
{
    ...
    */
    //à faire trouver une solution pour la connexion direct
    ...
    */
    ...
    if (layerc_size>0) {
	matrixXvector(neuc, neu2, sync, layerc_size, 0, layer2_size, 0, layerc_size, 1);
    ...
    {
    	matrixXvector(neu1, neu2, syn1, layer1_size, 0, layer2_size, 0, layer1_size, 1);
	...
	if (((counter%bptt_block)==0) || (independent && (word==0))) {
		...
		for (a=0; a<layer1_size; a++) neu1[a].er=neu1[a].er*neu1[a].ac*(1-neu1[a].ac);    //error derivation at layer 1
 		...
 		for (a=0; a<phRep_size; a++){
		    if (phRep[bptt_history[step]][a]==1){
                        //printf("%d \n",a); 
			for (b=0; b<layer1_size; b++) {
		...
		if ((counter%10)==0) {
	        ...
	        //printf("step %d bptt+bptt_block-2 %d \n",step, bptt+bptt_block-2);
		if ((counter%10)==0) {
...
{
    int a, b, word, last_word, wordcn, tag, last_tag;
    char log_name[200];
    FILE *fi, *flog, *fiTag;
    clock_t start, now;
    ///printf("lay0 %d lay1 %d layc %d lay2 %d vocab %p \n",layer0_size,layer1_size,layerc_size,layer2_size,vocab);
	...
	restoreNet();
	    ...
	    if (vocab!=NULL)free(vocab);
            ...
            buildPhRepresentation();


            //initNet();
    } else {
        ///printf("trainet lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p vocab %p \n",layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2,vocab);
        learnVocabFromTrainFile();
        ///for(a=0; a<vocab_size; a++) 
        ///printf("Vocab %s \n",vocab[a].word );
        //exit(0);
        //Modif 10/09 construire la represetation phrastique des mots
		buildPhRepresentation(); 
        ...
        //Modif 10/09 charger le tagset
        loadTagSet();      
  
        /*for(a=0; a<tagSet_size; a++)
        printf("%s %d \n",tagSet[a].tag,tagSet_size); 
        exit(1);*/        
        /*printf("tagSet: %s\n",tagSet_file);
        exit(1);*/
        ...
        //Modif 11/09 init net "taille couche entrée et couche sortiée"
	initNet();
	iter=0;
    }
    ...
    //saveNet();
    ///printf("Iter lay0 %d lay1 %d layc %d lay2 %d  \n",layer0_size,layer1_size,layerc_size,layer2_size);
    while (1) {
        ...
        while (1) {
            ...
            //printf("computeNet word %s lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p vocab %p \n",vocab[word].word,layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2,vocab);    
            computeNet(word, tag);      //compute probability distribution
            
            //if (feof(fi) || feof(fiTag)) break;        //end of file: test on validation data, iterate till convergence
            
            if (tag!=-1) logp+=log10(neu2[tag].ac);
		...
		for (a=bptt+bptt_block-1; a>0; a--) for (b=0; b<layer1_size; b++) {
            ...
            //
            ///printf("learnNet word %s lay0 %d lay1 %d layc %d lay2 %d  lay0 %p lay1 %p layc %p lay2 %p vocab %p history %p\n",vocab[word].word,layer0_size,layer1_size,layerc_size,layer2_size,neu0,neu1,neuc,neu2,vocab,history);
            learnNet(word, tag);
            ///printf("learnnet out \n");
            copyHiddenLayerToInput();

            if (word!=-1){
                for (a=0; a<phRep_size; a++) neu0[a].ac=0;  //delete previous activation
            }   

                      
            ///printf("\n\n!!! trainnet call avant !!!\n");
            word=readWordIndex(fi);     //read next word
            if (feof(fi) || feof(fiTag)) break; //{printf("feof(fi) || feof(fiTag)\n");break;}
            ///printf("!!!trainnet call !!! %d \n\n",word);
            //Modif 11/09 get word tag
            if (word!=0) tag=readTagIndex(fiTag,word);
            else tag=0;
            if (word==-1) word=0; 
            if (tag==-1) tag=0;          
	...
	if (flog==NULL) {
        ...
        while (1) {
            ///printf("1 \n");
            computeNet(word, tag);      //compute probability distribution
            if (feof(fi)) break; ///{printf("validation feof(fi)\n"); break;}	        //end of file: report LOGP, PPL
            ///printf("2 \n");
    	    if (word!=-1) {
    		logp+=log10(neu2[tag].ac);
        	wordcn++;
    	    }
            ///printf("3 \n");
            /*if (word!=-1)
...
void CRnnLM::testNet()
{
    int a, b, word, last_word, wordcn, Max, Count;
    FILE *fi, *flog, *lmprob=NULL;
    real prob_other, log_other, log_combine;
    char word_txt[MAX_STRING];      
    double d, neurTag; 
    restoreNet();    
    if (use_lmprob) {
    ...
    learnVocabFromTrainFile();
    printf("vocab ok\n");
    //Modif build target phRepresentation
    buildPhRepresentation();
    printf("PhRepr ok\n");
    /*for(a=0; a<vocab_size;a++)
       printf("%s \n",vocab[a].word);
    ...
    while (1) {
        ...
        //printf("Comput net avant\n");  
        computeNet(last_word, 0);		//compute probability distribution
        //printf("Comput net apres\n");
        if (feof(fi)) break;		//end of file: report LOGP, PPL
        
        neurTag = neu2[0].ac;
        Max=0; 
        for (Count=1; Count<layer2_size; Count++){
              //printf("%f \n",neu2[Count].ac);
             if (neurTag < neu2[Count].ac){
             neurTag = neu2[Count].ac;
             Max=Count;
             }
        }          
        if (last_word==-1){ last_word=0; printf("%s\t%s\t<UNK>\n",word_txt,tagSet[Max].tag);}
        if (last_word!=0) printf("%s\t%s\n",word_txt,tagSet[Max].tag);
	/*
	if (last_word!=0){
	printf("%s",word_txt);
	for (Count=0; Count<layer2_size; Count++)
		printf("\t%s\t%f",tagSet[Count].tag,neu2[Count].ac);
	printf("\n");	
	}*/
        ...
        if ((last_word!=-1) || (prob_other>0)) {
    		...
    		logp+=log10(neu2[Max].ac);
        	log_combine+=log10(neu2[Max].ac*lambda + prob_other*(1-lambda));
    	    }
        ...
        if (dynamic>0) {
            ...
            //
            alpha=dynamic;
    	    learnNet(last_word, Max);    //dynamic update
    	}
        copyHiddenLayerToInput();
        
        readWord(word_txt, fi); 
        last_word=searchVocab(word_txt); 
